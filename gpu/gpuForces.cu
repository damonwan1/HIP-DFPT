      ...
      int* load_balanced_matrix)
{
   ...
   if (loadBalancedMatrix) {
      ...
      HANDLE_CUBLAS(hipblasSetVector(
            1, sizeof(double),
...
void FORTRAN(forces_destroy_gpu)()
{
   ...
   HANDLE_CUDA(hipFree(dev_partition));
   HANDLE_CUDA(hipFree(dev_waveComputeA));
   HANDLE_CUDA(hipFree(dev_forcesShell));

   HANDLE_CUDA(hipFree(dev_hTimesPsi));
   HANDLE_CUDA(hipFree(dev_dWave));

   if (ggaForcesOn || relAtomicZORA || useASJacInPulay) {
      HANDLE_CUDA(hipFree(dev_wave));
   }

   if (ggaForcesOn) {
      HANDLE_CUDA(hipFree(dev_xcGradientDeriv));
      HANDLE_CUDA(hipFree(dev_hessianBasisWave));
      HANDLE_CUDA(hipFree(dev_gradientBasisWave));
      HANDLE_CUDA(hipFree(dev_matrixTmp1));
      HANDLE_CUDA(hipFree(dev_matrixTmp2));
   }

   if (metaGGAForcesOn) {
      HANDLE_CUDA(hipFree(dev_xcTauDeriv));
      HANDLE_CUDA(hipFree(dev_matrixTmp1MGGA));
      HANDLE_CUDA(hipFree(dev_matrixTmp2MGGA));
   }

   if (useAnalyticalStress) {
      HANDLE_CUDA(hipFree(dev_asStrainDerivWave));
      HANDLE_CUDA(hipFree(dev_asStrainDerivWaveShell));
      if (useASJacInPulay) {
         HANDLE_CUDA(hipFree(dev_asJacPotKinTimesPsi));
      }
      if (ggaForcesOn) {
         HANDLE_CUDA(hipFree(dev_matrixTmp3));
         HANDLE_CUDA(hipFree(dev_asHessianTimesXCDerivGGA));
      }
      if (metaGGAForcesOn) {
         HANDLE_CUDA(hipFree(dev_asHessianTimesXCDerivMGGA));
      }
      if (relAtomicZORA) {
         HANDLE_CUDA(hipFree(dev_asStrainDerivKineticWave));
         HANDLE_CUDA(hipFree(dev_asStrainDerivWaveShellTrans));
      }
   }

   if (relAtomicZORA) {
      HANDLE_CUDA(hipFree(dev_dHTimesPsi));
   }

   if (loadBalancedMatrix) {
      HANDLE_CUDA(hipFree(dev_insIdx));
      HANDLE_CUDA(hipFree(dev_densMat));
      HANDLE_CUDA(hipFree(dev_permuteComputeByAtom));
      HANDLE_CUDA(hipFree(dev_forceValues));
      HANDLE_CUDA(hipFree(dev_theNumberOneHaHaHa));
      HANDLE_CUDA(hipFree(dev_forceComponent));
      HANDLE_CUDA(hipFree(dev_sumForces));
      if (useAnalyticalStress) {
         HANDLE_CUDA(hipFree(dev_asPulayStressLocal));
      }
   }
}
      ...
      int* n_compute_for_atom)
{
   ...
   if (loadBalancedMatrix) {
      hipLaunchKernelGGL(updateSumForcesLoadBalanced, dim3(grids), dim3(threads), 0, 0, 
            *n_compute,
   ...
   hipblasPointerMode_t pMode;
   HANDLE_CUBLAS(hipblasGetPointerMode(cublasHandle, &pMode));
   HANDLE_CUBLAS(hipblasSetPointerMode(cublasHandle,HIPBLAS_POINTER_MODE_DEVICE));
   ...
   for (unsigned int iAtom = 1; iAtom <= nAtoms; iAtom++)  {
      ...
      // See the AS tensor indexing for an explanation on usage of hipblasDdot.
      HANDLE_CUBLAS(hipblasDdot(
            cublasHandle,
      ...
      hipLaunchKernelGGL(sumForcesAdd, dim3(1), dim3(1), 0, 0, dev_sumForces + target, dev_forceComponent);
      CHECK_FOR_ERROR();
      offsetForceValues += nComputeCurrent * *n_compute;
   }
   HANDLE_CUBLAS(hipblasSetPointerMode(cublasHandle,pMode));
}
      ...
      int* n_compute_c)
{
   ...
   HANDLE_CUBLAS(hipblasDcopy(cublasHandle,
         (*n_compute_c)*(*n_compute_c),
      ...
      int* n_compute_for_atom)
{
   ...
   // used.
   if (loadBalancedMatrix) {
      hipLaunchKernelGGL(asUpdateStressLoadBalanced, dim3(grids), dim3(threads), 0, 0, 
            *n_compute,
            *i_calculate_dimension,
            dev_insIdx,
            dev_densMat + densMatOffset,
   ...
   // This allows us to use hipblasDdot to perform a reduction.  I've (WPH) done
   ...
   HANDLE_CUBLAS(hipblasDdot(
         ...
         *n_compute * *n_compute,
         ...
         dev_forceComponent));
   // Finally, add the results to the current value for the force component
   hipLaunchKernelGGL(sumForcesAdd, dim3(1), dim3(1), 0, 0, dev_asPulayStressLocal + target, dev_forceComponent);
   CHECK_FOR_ERROR();
   HANDLE_CUBLAS(hipblasSetPointerMode(cublasHandle,pMode));
