...
namespace gpuMR {
    ...
    int *d_i_basis;
    double **d_matrix_list[3]; // work array for hipblasDgemmBatched
                       ...
                       const int &matrix_wave_size_f)
{
    ...
    // Allocations
    hipMalloc((void**) &d_wave, wave_size*sizeof(double));
    hipMalloc((void**) &d_matrix_wave, matrix_wave_size*sizeof(double));
    hipMalloc((void**) &d_matrix_batch, matrix_batch_size*sizeof(double));
    hipMalloc((void**) &d_matrix_batch_GIAO,
               matrix_batch_GIAO_size*sizeof(double));
    hipMalloc((void**) &d_r_mn_basis, r_mn_basis_size*sizeof(double));
    hipMalloc((void**) &d_matrix_packed, matrix_packed_size*sizeof(double));
    hipMalloc((void**) &d_i_basis, i_basis_size*sizeof(int));
    hipMalloc((void**) &d_basis_glb_to_loc, basis_glb_to_loc_size*sizeof(int));
    ...
    hipLaunchKernelGGL(d_zero_matrix, dim3(nBlocks), dim3(nThreads), 0, 0, d_matrix_packed, matrix_packed_size);
    // Copy basis_glb_to_loc to gpu
    hipblasSetVector(basis_glb_to_loc_size, sizeof(int),
                    basis_glb_to_loc, 1, d_basis_glb_to_loc, 1);
    for (int i = 0; i < 3; ++i)
        hipMalloc((void**) &d_matrix_list[i], 9*sizeof(double*));
                       ...
                       const int &max_dims)
{
    using namespace gpuMR;
    if (i_dir == 1) {
        hipblasSetVector(n_compute*n_points, sizeof(double),
                        wave, 1, d_wave, 1);
        hipblasSetVector(max_dims*n_compute*n_points, sizeof(double),
                        matrix_wave, 1, d_matrix_wave, 1);
    }
    const double alpha = 1.0;
    const double beta = 0.0;
    hipblasDgemm(cublasHandle, HIPBLAS_OP_N, HIPBLAS_OP_T,
    ...
    if (i_symmetrization == 1) {
        hipLaunchKernelGGL(symmetrize, dim3(nBlocks), dim3(nThreads), 0, 0, d_matrix_batch, n_compute);
    } else if (i_symmetrization == 2) {
        hipLaunchKernelGGL(antisymmetrize, dim3(nBlocks), dim3(nThreads), 0, 0, d_matrix_batch, n_compute);
    }
}
...
{
    using namespace gpuMR;
    hipblasSetVector(n_compute, sizeof(int), &i_basis, 1, d_i_basis, 1);
    nThreads = dim3(maxThreadsX, maxThreadsY);
    nBlocks  = dim3((n_compute + nThreads.x - 1)/nThreads.x,
                    (n_compute + nThreads.y - 1)/nThreads.y);
    hipLaunchKernelGGL(d_batch_to_packed, dim3(nBlocks), dim3(nThreads), 0, 0, d_matrix_packed,
                                            d_matrix_batch,
                                            n_compute,
                                            starting_point,
                                            d_i_basis,
                              ...
                              const int &matrix_packed_ld)
{
    ...
    nBlocks  = dim3((n_compute + nThreads.x - 1)/nThreads.x,
                    (n_compute + nThreads.y - 1)/nThreads.y);
    hipLaunchKernelGGL(d_batch_to_packed_full, dim3(nBlocks), dim3(nThreads), 0, 0, d_matrix_packed,
                                             d_matrix_batch,
                                             n_compute,
                                             starting_point,
                                             d_i_basis,
                       ...
                       const int &n_spins)
{
    ...
    if (i_dim == 1) {
        // Copy data to gpu
        hipblasSetVector(n_compute*n_points, sizeof(double), wave, 1, d_wave, 1);
        hipblasSetVector(matrix_wave_size_factor*n_spins*n_compute*n_points,
                        sizeof(double), matrix_wave, 1, d_matrix_wave, 1);
        hipblasSetVector(3*batch_size, sizeof(double), r_mn_basis, 1,
                        d_r_mn_basis, 1);
    ...
    if (mult_R_mn_both) {
        if (compact_indexing) {
            if (i_dim == 1) {
                ...
                for (int i = 0; i < 3; ++i)
                    hipblasSetVector(6, sizeof(double*),
                ...
                if (do_transpose) {
                    hipLaunchKernelGGL(d_do_transpose, dim3(nBlocks), dim3(nThreads), 0, 0, 6, n_compute, d_matrix_batch_GIAO);
                }
            }
            if (n_dims == 9) {
                switch(i_dim) {
                case 1:
                    ...
                    hipLaunchKernelGGL(d_operate4, dim3(nBlocks), dim3(nThreads), 0, 0, n_compute, d_r_mn_basis,
                ...
                case 2:
                    ...
                    hipLaunchKernelGGL(d_operate1, dim3(nBlocks), dim3(nThreads), 0, 0, n_compute, d_r_mn_basis,
                ...
                case 1:
                    ...
                    hipLaunchKernelGGL(d_operate3, dim3(nBlocks), dim3(nThreads), 0, 0, n_compute, d_r_mn_basis,
                         l_1, r_GIAO_11, r_mn_11, r_GIAO_12, r_mn_12,
                         l_2, r_GIAO_21, r_mn_21, r_GIAO_22, r_mn_22,
            ...
            if (i_dim == 1) {
                ...
                for (int i = 0; i < 3; ++i)
                    hipblasSetVector(9, sizeof(double*),
                                    matrix_list[i], 1, d_matrix_list[i], 1);
                hipblasDgemmBatched
                    (cublasHandle, HIPBLAS_OP_N, HIPBLAS_OP_T,
                     n_compute, n_compute, n_points,
                ...
                if (do_transpose) {
                    hipLaunchKernelGGL(d_do_transpose, dim3(nBlocks), dim3(nThreads), 0, 0, 9, n_compute, d_matrix_batch_GIAO);
                }
            }
            if (n_dims == 9) {
                switch(i_dim) {
        ...
        if (compact_indexing) {
            ...
            case 1:
                ...
                for (int i = 0; i < 3; ++i)
                    hipblasSetVector(2, sizeof(double*),
                                    matrix_list[i], 1, d_matrix_list[i], 1);
                hipblasDgemmBatched
                    (cublasHandle, HIPBLAS_OP_N, HIPBLAS_OP_T,
                     n_compute, n_compute, n_points,
                ...
                // Cross products
                hipLaunchKernelGGL(d_cross_product_modified, dim3(nBlocks), dim3(nThreads), 0, 0, n_compute, d_r_mn_basis, d_matrix_batch_GIAO,
            ...
            hipblasGetVector(batch_size, sizeof(double),
            ...
            case 7:
                ...
                if (do_transpose) {
                    hipLaunchKernelGGL(d_do_transpose, dim3(nBlocks), dim3(nThreads), 0, 0, 3, n_compute, d_matrix_batch_GIAO);
                }
                // Cross products
                hipLaunchKernelGGL(d_cross_product, dim3(nBlocks), dim3(nThreads), 0, 0, n_compute, d_r_mn_basis, d_matrix_batch_GIAO,
...
{
    using namespace gpuMR;
    hipblasGetVector(matrix_packed_size, sizeof(double), d_matrix_packed, 1,
                    matrix_packed, 1);
}
...
{
    using namespace gpuMR;
    hipFree(d_matrix_batch);
    hipFree(d_matrix_batch_GIAO);
    hipFree(d_r_mn_basis);
    hipFree(d_wave);
    hipFree(d_matrix_wave);
    hipFree(d_matrix_packed);
    hipFree(d_basis_glb_to_loc);
    hipFree(d_i_basis);
    for (int i = 0; i < 3; ++i)
        hipFree(d_matrix_list[i]);
}
