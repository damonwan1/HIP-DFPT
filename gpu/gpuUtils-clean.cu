...
void rho_begin_()
{
    ...
    _FW_(double, NULL, ((rho_param.l_ylm_max + 1) * (rho_param.l_ylm_max + 1) * n_max_compute_atoms), scaled_dylm_dphi_tab__, 0); // 反正这个和上面那个暂时无用
...
#define _WRITE_(type, var, size, var_out, hip_mem_flag) \
    {                                                   \
        writeArrayToFile(var, size, #var_out);          \
    }
...
void h_begin_()
{
    ...
    _FW_(double, NULL, (globalSize[0] / localSize[0]) * ((n_max_batch_size + 127) / 128 * 128) * ((n_max_compute_ham + 127) / 128 * 128) + 256 + 16 * n_max_compute_ham,
         wave_t__, 0); // 多加 128 为了避免 TILE 后越界, 长宽按 128 对齐
    ...
    if (htimes == 0)
    {
        ...
        // for (int i = 0; i < n_my_batches_work_h; ++i)
        // {
        //     batches.push_back({i, H_param.n_batch_centers_all_batches[i]});
        // }
        std::vector<Bucket> buckets = distributeBatches(batches);
        int new_id = 0;
        for (int i = 0; i < S; i++)
...
void sum_up_begin_()
{
    ...
    _FW_(double, NULL, globalSize[0] * 1, vector, 0); // 留下这个，不过这个理应已经废弃
    ...
    setKernelArgs(sum_up::args, remember_arg_Fp_function_spline, sizeof(hipDeviceptr_t), &hip_buf_com.Fp_function_spline_slice);      // TODO 赋值位置可能得改
    setKernelArgs(sum_up::args, remember_arg_Fp_function_spline + 1, sizeof(hipDeviceptr_t), &hip_buf_com.Fpc_function_spline_slice); // TODO 赋值位置可能得改
...
#undef batches_size_sumup
    ...
    for (int i_center_tile = 0; i_center_tile < n_centers_hartree_potential; i_center_tile += i_center_tile_size)
    {
        ...
        for (int vid = 0; vid < vnum; vid++)
        {
            ...
            setKernelArgs(sum_up::args, sum_up::arg_index++, sizeof(int), &i_valid_points[vid]); // TODO 注意这个 ！！！, 有没有 partition_tab 的预判不同 !
