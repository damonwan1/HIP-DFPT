...
__device__ void prune_radial_basis_p2_c_(
...
#define atom_index_inv(i) atom_index_inv[((i)-1) * lsize + lid]
    ...
    while (i_compute <= n_compute)
    {
        ...
        // i_fn = i_basis_fns_inv(basis_fn(Cbasis_to_basis(i_basis_1)),
        // Cbasis_to_center(i_basis_1)); i_fn =
        // i_basis_fns_inv(basis_fn(Cbasis_to_basis(i_basis_1)),
        // private_center_to_atom[Cbasis_to_center(i_basis_1)]); i_fn =
        // i_basis_fns_inv(basis_fn(Cbasis_to_basis(i_basis_1)),
        ...
        // if (gid == 0)
        // {
        //     printf("gid = %d, i_compute = %d, i_fn = %d, l_aux = %d\n", gid, i_compute, i_fn, l_aux);
        // }
        ...
        else if (wave_index[i_fn - 1] == 0)
        {
            // if (gid == 0)
            // {
            //     printf("gid = %d, i_compute = %d, n_compute = %d, i_fn = %d, l_aux = %d\n", gid, i_compute, n_compute, i_fn, l_aux);
            // }
    ...
    // if (gid == 0)
    // {
    //     printf("prune basis gid %d l_count: \n", gid);
    //     for (int i = 0; i < 10; i++)
    //         printf("%d, ", l_count[i]);
    //     printf("\n");
    // }
    ...
    *n_zero_compute_ = n_zero_compute;
    // if (gid == 0)
    //     printf("end prune_radial_basis_p2_c_ : gid is : %d\n", gid);
...
__device__ void tab_gradient_ylm_p0_c_2(
    ...
    // outer
    ,
    double *dir_tab, const int *species_center)
{
#define species_center(i) species_center[(i)-1]
...
__device__ void evaluate_waves_p2_c_2(
    ...
    // tmp
    ,
    double *aux_radial, int array_factor)
{
    int gid = blockIdx.x * blockDim.x + threadIdx.x;
    int gsize = gridDim.x * blockDim.x;
...
// inline void atomicAdd_g_f(volatile double *addr, double val)
// {
//     union
//     {
//         unsigned long u64;
...
//         current.u64 = atomicCAS((volatile unsigned long *)addr,
...
#define first_order_H_dense(i, j, k) \
    ...
    if (n_spin == 1)
    {
        ...
        // double one = 1.0;
        // double zero = 0.0;
        // dgemm("T", "N", &n_compute_c, &n_compute_c, &n_points, &one, contract,
        // &n_points,
...
__global__ void integrate_first_order_h_sub_tmp2_(
    ...
    double *first_order_H_dense__, int max_n_batch_centers
    // test
...
#define batch_point_to_i_full_point(i, j) batch_point_to_i_full_point[(i)-1 + n_max_batch_size * ((j)-1)]
    ...
    //  double *wave = wave__ + gid * n_max_compute_ham;
    double *wave_group = wave__ + bid * n_max_batch_size * n_max_compute_ham;
    double *i_r = i_r__ + gid * n_max_compute_atoms;
    double *trigonom_tab = trigonom_tab__ + gid * 4 * n_max_compute_atoms;
    ...
    //  double *kinetic_wave = kinetic_wave__ + gid * n_max_compute_fns_ham;
    double *grid_coord_group = grid_coord__ + bid * n_max_batch_size; // 共用性类似 wave
                                                                      //  double *H_times_psi_group = H_times_psi__ + bid * (n_max_compute_ham * lsize * n_spin);   // 共用性类似 wave
                                                                      //  double *T_plus_V = T_plus_V__ + gid * n_max_compute_atoms * n_basis_fns; // 有风险, n_max_compute_atoms 是猜的

    //  double *contract_group = contract__ + bid * n_max_batch_size * n_max_compute_ham;
    //  double *wave_t_group = wave_t__ + bid * n_max_batch_size * n_max_compute_ham;
    double *first_order_H_dense_group = first_order_H_dense__ + bid * n_max_compute_ham * n_max_compute_ham * n_spin;
    // if (threadIdx.x == 0)
    // if(gid == 0) printf("before loop : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
    for (int i_my_batch = bid + 1; i_my_batch <= n_my_batches_work_h; i_my_batch += (gsize / lsize))
    {
        ...
        if (n_compute_c > 0)
        {
            ...
            for (int i_point_div = 0; i_point_div < ((n_points_all_batches[i_my_batch - 1] + lsize - 1) / lsize); i_point_div++)
            {
                ...
                if (i_point <= n_points_all_batches[i_my_batch - 1])
                {
                    ...
                    // int rad_index[const_cast<const int &>(n_max_compute_atoms)];
                    // double one_over_dist_tab[const_cast<const int &>(n_max_compute_atoms)];
                    ...
                    // if (threadIdx.x == 0)
                    // if(gid == 0) printf("before tab_local_geometry_p2_c_ : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
                    ...
                    // if (threadIdx.x == 0)
                    // if(gid == 0) printf("before tab_gradient_ylm_p0_c_2 : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
                    ...
                    // if (threadIdx.x == 0)
                    // if(gid == 0) printf("before evaluate_radial_functions_p0_c_ : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
                    evaluate_radial_functions_p0_c_(
                        ...
                        ZERONULL);
                    //     // if (threadIdx.x == 0)
                    //     // if(gid == 0) printf("before evaluate_waves_p2_c_2 : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
                    ...
                    // if (gid == 0)
                    //     printf("after evaluate_waves_p2_c_2 gid=%6d lid=%3d i_my_batch=%4d i_point_div=%3d i_point=%3d n_point=%4d\n",
            ...
            // if (threadIdx.x == 0)
            // if(gid == 0) printf("before evaluate_first_order_h_polar_reduce_memory_c_ : gid is : %d, lid is : %d, bid is : %d\n", gid, lid, bid);
