      ...
      int* use_density_gradient)
{
   ...
   if (*n_max_compute_dens > 0) {
     ...
     hipLaunchKernelGGL(gpuInitVectorDensity, dim3(nBlocks), dim3(nThreads), 0, 0, dev_wave, 0.0, dim);
     CHECK_FOR_ERROR();

     dim = *n_max_compute_dens * *n_max_compute_dens;
     nThreads = dim3(maxThreads);
     nBlocks = dim3((dim + nThreads.x - 1)/nThreads.x);
     hipLaunchKernelGGL(gpuInitVectorDensity, dim3(nBlocks), dim3(nThreads), 0, 0, dev_densityMatrix, 0.0, dim);
     CHECK_FOR_ERROR();
     ...
     if (*use_density_gradient != 0) {
        ...
        nThreads = dim3(maxThreads);
        nBlocks = dim3((dim + nThreads.x - 1)/nThreads.x);
        hipLaunchKernelGGL(gpuInitVectorDensity, dim3(nBlocks), dim3(nThreads), 0, 0, 
...
void FORTRAN(density_destroy_gpu)()
{
   ...
   HANDLE_CUDA(hipFree(dev_rho));
   HANDLE_CUDA(hipFree(dev_deltaRho));
   HANDLE_CUDA(hipFree(dev_tempRho));
   HANDLE_CUDA(hipFree(dev_rhoChange));
   HANDLE_CUDA(hipFree(dev_partitionTab));
   HANDLE_CUDA(hipFree(dev_hartreePartitionTab));
   HANDLE_CUDA(hipFree(dev_wave));
   HANDLE_CUDA(hipFree(dev_densityMatrix));
   HANDLE_CUDA(hipFree(dev_batchPoint2FullPoint));
   HANDLE_CUDA(hipFree(dev_work));
   HANDLE_CUDA(hipFree(dev_resultMat));

   HANDLE_CUDA(hipFree(dev_gradientBasisWave));
   HANDLE_CUDA(hipFree(dev_tempRhoGradient));
   HANDLE_CUDA(hipFree(dev_deltaRhoGradient));
   HANDLE_CUDA(hipFree(dev_rhoGradient));
}
      ...
      int* n_basis_compute) //maximum number of relevant basis functions
{
   ...
   if (*n_points > 0 && *n_basis_compute > 0) {
      ...
      hipLaunchKernelGGL(gpuDiag, dim3(nBlocks), dim3(nThreads), 0, 0, 
      ...
      int* n_spin)             // number of Spin Channels
{
   ...
   hipLaunchKernelGGL(generateBatchPoint2FullPoint, dim3(1), dim3(1), 0, 0, 
   ...
   if (*n_points_compute > 0) {
      ...
      hipLaunchKernelGGL(distributeDensityUpdate, dim3(nBlocks), dim3(nThreads), 0, 0, 
            *n_points_compute,
      ...
      int* n_spin)
{
   ...
   if (*n_full_points > 0) {
      ...
      hipLaunchKernelGGL(updateRhoChange, dim3(nBlocks), dim3(nThreads), 0, 0, 
      ...
      int* n_spin)             // number of Spin Channels
{
   ...
   if (*n_points_compute > 0) {
      ...
      hipLaunchKernelGGL(distributeGradDensityUpdate, dim3(nBlocks), dim3(nThreads), 0, 0, 
            *n_points_compute,
            *n_full_points, *n_spin, *i_spin, dev_batchPoint2FullPoint,
