...
void FORTRAN(hamiltonian_destroy_gpu)()
{
   ...
   HANDLE_CUDA(hipFree(dev_hamiltonianShell));
   ...
   HANDLE_CUDA(hipFree(dev_leftSideOfMGGADotProduct));
   HANDLE_CUDA(hipFree(dev_gradientBasisWaveStore));

   HANDLE_CUDA(hipFree(dev_zoraVector1));
   HANDLE_CUDA(hipFree(dev_zoraVector2));

   // Hamiltonian indexing quantities
   HANDLE_CUDA(hipFree(dev_hamiltonian));
   HANDLE_CUDA(hipFree(dev_map));
}
      ...
      int* dim_map) // dimension of map
{
   ...
   hipLaunchKernelGGL(insertInHamiltonianViaMap, dim3(grids), dim3(threads), 0, NULL, 
         dev_hamiltonian,
         *dim_hamiltonian,
      ...
      int* n_compute_c) // number of nonzero basis functions
{
   ...
   hipLaunchKernelGGL(insertShellInMatrix, dim3(nBlocks), dim3(nThreads), 0, 0, 
         dev_hamiltonian,
         dev_hamiltonianShell,
         dev_map,
         *ld_matrix,
